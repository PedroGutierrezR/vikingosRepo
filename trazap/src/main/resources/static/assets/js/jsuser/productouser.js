				...
				tablaProductos.bootstrapTable({
					...
					},
					{
						...
						formatter: function(value, row, index) {
							return [
								"<a class='like' data-toggle='modal' data-target='#modalEditarProducto' onclick='onClickEditarProducto(\"" + JSON.stringify(row).split('"').join('\\"') + "\");getTipoProducto(editarOption1user);getCategoria(editarOption2user);' title='Like'>",
...
$("#idBtnEditarProducto").click(function() {
	...
	if (validaFormEditarProducto()) {
		...
		dataProducto = {
			...
			"categoriaProducto": {
				"idCategoriaProducto": $("#idSelEditarCategoriaProducto").val()
			},
			"tipoProducto": {
				"idTipoProducto": $("#idSelEditarTipoProducto").val()
			}
		}
